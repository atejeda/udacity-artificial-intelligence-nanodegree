...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        if len(self.a_levels) - 1 < level:
            self.a_levels.append(set())

        literals = [node.symbol for node in self.s_levels[level]]
        literals_pos = [node.symbol for node in self.s_levels[level] if node.is_pos]
        literals_neg = [node.symbol for node in self.s_levels[level] if not node.is_pos]

        for action in self.all_actions:
            # print("Level Literals Using {}".format(literals))
            # print("Level {}: action {}, \n\t pre+ {}\n\t pre- {}"\
            #     .format(level, action, action.precond_pos, action.precond_neg))

            # why the preconditions in false needs to be tested in search but not
            # on planning graph?
            # need to keep track on the negatives as well? -> is related to the
            # mutex ??

            add_action = True

            for clause in action.precond_pos:
                if clause not in literals_pos:
                    add_action = False
                    break

            for clause in action.precond_neg:
                if clause not in literals_neg:
                    add_action = False
                    break
    ...
    def add_literal_level(self, level):
        ...
        if len(self.s_levels) - 1  < level:
            self.s_levels.append(set())

        for node_a in self.a_levels[level-1]:
            self.s_levels[level] |= node_a.effnodes
            node_a.children |= node_a.effnodes
            for node_s in node_a.effnodes:
                node_s.parents.add(node_a)

        # print("Level Literals {}: {}"\
        #     .format(level, [n.symbol for n in self.s_levels[level]]))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        action1_eff_pos = action1.effect_add
        action1_eff_neg = action1.effect_rem

        action2 = node_a2.action
        action2_eff_pos = action2.effect_add
        action2_eff_neg = action2.effect_rem

        for eff in action1_eff_pos:
            if eff in action2_eff_neg:
                return True

        for eff in action2_eff_pos:
            if eff in action1_eff_neg:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        action1_eff_pos = action1.effect_add
        action1_eff_neg = action1.effect_rem
        action1_pre_pos = action1.precond_pos
        action1_pre_neg = action1.precond_neg

        action2 = node_a2.action
        action2_eff_pos = action2.effect_add
        action2_eff_neg = action2.effect_rem
        action2_pre_pos = action2.precond_pos
        action2_pre_neg = action2.precond_neg

        for eff in action1_eff_pos:
            if eff in action2_pre_neg:
                return True

        for eff in action1_eff_neg:
            if eff in action2_pre_pos:
                return True

        for eff in action2_eff_pos:
            if eff in action1_pre_neg:
                return True

        for eff in action2_eff_neg:
            if eff in action1_pre_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #     print("node1 parent symbol {}, mutex {}".format(p.symbol, [n.symbol for n in p.mutex]))

        # for p in node_a2.parents:
        #     print("node2 parent symbol {}, mutex {}".format(p.symbol, [n.symbol for n in p.mutex]))
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        #
        # if each possible pair of actions 
        #   that could achieve the two literal is mutually exclusive. 
        # This condition is called inconsistent support (fragment) 

        # check that the literals aren't the opposite s1 != ~s2

        for p1 in node_s1.parents:
            for p2 in node_s2.parents:
                if not p1.is_mutex(p2): # -> review this
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            for level,node in enumerate(self.s_levels):
                if goal in [literal.symbol for literal in node if literal.is_pos]:
                    level_sum += level
                    break
        return level_sum
